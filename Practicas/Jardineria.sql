...
SELECT CL.NOMBRE_CLIENTE,(E.NOMBRE || E.APELLIDO1 ) AS NOMBRE_REPRESENTANTE
FROM CLIENTE CL, EMPLEADO E
WHERE CL.CODIGO_EMPLEADO_REP_VENTAS = E.CODIGO_EMPLEADO;

--2 Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus representantes de ventas.
SELECT DISTINCT CL.NOMBRE_CLIENTE, E.NOMBRE AS REPRESENTANTE
FROM CLIENTE CL, EMPLEADO E, PAGO P
WHERE CL.CODIGO_EMPLEADO_REP_VENTAS = E.CODIGO_EMPLEADO
AND P.CODIGO_CLIENTE = CL.CODIGO_CLIENTE ;

--3 Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de sus representantes de ventas.
SELECT DISTINCT c.NOMBRE_CLIENTE, e.NOMBRE || ' ' || e.APELLIDO1 AS REPRESENTANTES
FROM CLIENTE c, EMPLEADO e, PAGO p 
WHERE c.CODIGO_EMPLEADO_REP_VENTAS = e.CODIGO_EMPLEADO
AND c.CODIGO_CLIENTE = p.CODIGO_CLIENTE (+)
AND p.CODIGO_CLIENTE IS NULL 
GROUP BY c.NOMBRE_CLIENTE, e.NOMBRE, e.APELLIDO1 ;

--4 Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT DISTINCT CL.NOMBRE_CLIENTE,E.NOMBRE AS REPRESENTANTE, O.CIUDAD AS CIUDAD_OFICINA
FROM CLIENTE CL, EMPLEADO E, PAGO P, OFICINA O
WHERE CL.CODIGO_EMPLEADO_REP_VENTAS = E.CODIGO_EMPLEADO
AND P.CODIGO_CLIENTE = CL.CODIGO_CLIENTE
...
WHERE c.CODIGO_EMPLEADO_REP_VENTAS = e.CODIGO_EMPLEADO 
AND e.CODIGO_OFICINA = o.CODIGO_OFICINA 
AND (UPPER(c.LINEA_DIRECCION1) LIKE '%FUENLABRADA%'
OR upper(c.CIUDAD) LIKE '%FUENLABRADA%'
OR upper(c.LINEA_DIRECCION2) LIKE '%FUENLABRADA%'
OR upper(c.REGION) LIKE '%FUENLABRADA%')
GROUP BY o.LINEA_DIRECCION1, o.LINEA_DIRECCION2;

--7 Devuelve el nombre de los clientes y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT DISTINCT CL.NOMBRE_CLIENTE, E.NOMBRE, O.CIUDAD
...
GROUP BY CL.NOMBRE_CLIENTE,E.NOMBRE, O.CIUDAD;

--8 Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.
SELECT DISTINCT E.NOMBRE, EM.NOMBRE AS JEFE
FROM EMPLEADO E, EMPLEADO EM
WHERE E.CODIGO_EMPLEADO = EM.CODIGO_JEFE;
...
SELECT DISTINCT PRO.GAMA, CL.NOMBRE_CLIENTE
FROM PRODUCTO PRO, CLIENTE CL, DETALLE_PEDIDO DE, PEDIDO P
WHERE CL.CODIGO_CLIENTE = P.CODIGO_CLIENTE 
AND P.CODIGO_PEDIDO = DE.CODIGO_PEDIDO 
AND PRO.CODIGO_PRODUCTO = DE.CODIGO_PRODUCTO;

--Otra forma
SELECT DISTINCT p.GAMA,gp.DESCRIPCION_TEXTO, c.CODIGO_CLIENTE
FROM CLIENTE c, PRODUCTO p, DETALLE_PEDIDO dp, PEDIDO p2, GAMA_PRODUCTO gp  
WHERE c.CODIGO_CLIENTE = p2.CODIGO_CLIENTE 
AND p2.CODIGO_PEDIDO = dp.CODIGO_PEDIDO 
AND dp.CODIGO_PRODUCTO = p.CODIGO_PRODUCTO
AND p.GAMA = gp.GAMA 
GROUP BY  c.CODIGO_CLIENTE, p.GAMA, gp.DESCRIPCION_TEXTO 
...
WHERE E.CODIGO_EMPLEADO = O.CODIGO_OFICINA (+);
--
SELECT e.NOMBRE
FROM EMPLEADO e, OFICINA o 
...
--Otra forma 
SELECT p2.CODIGO_PRODUCTO, p2.NOMBRE 
FROM PRODUCTO p2
WHERE p2.CODIGO_PRODUCTO NOT IN
...
AND upper (p.GAMA) LIKE 'FRUTALES');
...
SELECT COUNT (CL.CODIGO_CLIENTE) AS NUM_CLIENTES, CL.PAIS
FROM CLIENTE CL
GROUP BY PAIS;
...
SELECT CL.CODIGO_CLIENTE, MIN(P.FECHA_PAGO) AS PRIMER_PAGO ,MAX(P.FECHA_PAGO) AS ULTIMO_PAGO
FROM PAGO P, CLIENTE CL
WHERE P.CODIGO_CLIENTE = CL.CODIGO_CLIENTE
GROUP BY CL.CODIGO_CLIENTE,CL.NOMBRE_CLIENTE ;

--12 Calcula el número de productos diferentes que hay en cada uno de los pedidos.
SELECT DE.CODIGO_PEDIDO ,COUNT(DISTINCT DE.CODIGO_PRODUCTO)
FROM DETALLE_PEDIDO DE 
GROUP BY DE.CODIGO_PEDIDO;

--13 Calcula la suma de la cantidad total de todos los productos que aparecen en cada uno de los pedidos.
SELECT DE.CODIGO_PEDIDO, SUM(DE.CANTIDAD)
FROM DETALLE_PEDIDO DE
GROUP BY DE.CODIGO_PEDIDO;
...
SELECT sum (dp.PRECIO_UNIDAD * dp.CANTIDAD) AS BASE_IMPONIBLE, ((sum (dp.PRECIO_UNIDAD * dp.CANTIDAD)* 0.21))AS IVA, ((sum (dp.PRECIO_UNIDAD * dp.CANTIDAD)* 0.21) +  sum (dp.PRECIO_UNIDAD * dp.CANTIDAD) )AS TOTAL_FACTURADO  
FROM DETALLE_PEDIDO dp;

--16 La misma información que en la pregunta anterior, pero agrupada por código de producto.
SELECT dp.CODIGO_PRODUCTO, sum (dp.PRECIO_UNIDAD * dp.CANTIDAD) AS BASE_IMPONIBLE, ((sum (dp.PRECIO_UNIDAD * dp.CANTIDAD)* 0.21))AS IVA, ((sum (dp.PRECIO_UNIDAD * dp.CANTIDAD)* 0.21) +  sum (dp.PRECIO_UNIDAD * dp.CANTIDAD) )AS TOTAL_FACTURADO  
FROM DETALLE_PEDIDO dp
GROUP BY dp.CODIGO_PRODUCTO;
...
SELECT p.NOMBRE, sum(dp.CANTIDAD) AS UNIDADES_VENDIDAS, sum(dp.CANTIDAD * dp.PRECIO_UNIDAD) AS TOTAL_FACTURADO, (1.21 * sum(dp.CANTIDAD * dp.PRECIO_UNIDAD)) AS TOTAL_CON_IMPUESTOS
...
-- (Ten en cuenta que tendrás que calcular cuál es el número total de unidades que se han vendido de cada producto a partir 
--de los datos de la tabla detalle_pedido. Una vez que sepas cuál es el código del producto, puedes obtener su nombre fácilmente.)
SELECT p.NOMBRE
FROM DETALLE_PEDIDO dp, PRODUCTO p 
WHERE p.CODIGO_PRODUCTO = dp.CODIGO_PRODUCTO
...
FROM EMPLEADO E 
WHERE E.NOMBRE,E.APELLIDO1, E.APELLIDO2, E.EMAIL =
(SELECT NOMBRE FROM EMPLEADO WHERE NOMBRE LIKE 'Alberto');
...
SELECT CL.NOMBRE_CLIENTE, COUNT(DE.CODIGO_PEDIDO)
FROM CLIENTE CL, DETALLE_PEDIDO DE, PEDIDO PE
WHERE CL.CODIGO_CLIENTE = PE.CODIGO_PEDIDO 
AND DE.CODIGO_PEDIDO = PE.CODIGO_PEDIDO 
...
SELECT DISTINCT CL.NOMBRE_CLIENTE, P.TOTAL
...
WHERE CL.CODIGO_CLIENTE = PE.CODIGO_PEDIDO 
AND EXTRACT(YEAR FROM PE.FECHA_PEDIDO)=2008;
...
--de ventas y la ciudad donde esta? su oficina.
...
GROUP BY O.CIUDAD  ;
--alter session set "_oracle_script"=true;  
--CREATE USER Jardin IDENTIFIED BY Jardin;
--GRANT CONNECT, RESOURCE, DBA TO Jardin;
...
WHERE CL.CODIGO_CLIENTE = P.CODIGO_PEDIDO 
AND P.FECHA_ENTREGA > P.FECHA_ESPERADA;
...
--6 Devuelve un listado que muestre los empleados que no tienen una oficina asociada y los que no tienen un cliente asociado.
SELECT DISTINCT E.CODIGO_EMPLEADO
FROM EMPLEADO E, OFICINA O, CLIENTE CLI
WHERE E.CODIGO_EMPLEADO = O.CODIGO_OFICINA (+)
AND E.CODIGO_EMPLEADO = CLI.CODIGO_EMPLEADO_REP_VENTAS (+);
...
--9 Devuelve el código de empleado y el número de clientes al que atiende cada representante de ventas.
SELECT E.CODIGO_EMPLEADO, COUNT(CL.CODIGO_CLIENTE) AS NUM_CLIENTES
...
--13 Calcula la suma de la cantidad total de todos los productos que aparecen en cada uno de los pedidos.
SELECT DE.CODIGO_PEDIDO , SUM(DE.CANTIDAD)AS CANTIDAD_TOTAL
FROM DETALLE_PEDIDO DE
GROUP BY DE.CODIGO_PEDIDO;
...
AND upper (e.NOMBRE) LIKE 'ALBERTO'
AND upper(e.APELLIDO1) LIKE 'SORIA'; 
...
--1 Devuelve el listado de clientes indicando el nombre del cliente y cuántos pedidos ha realizado. 
--Tenga en cuenta que pueden existir clientes que no han realizado ningún pedido.
SELECT c.NOMBRE_CLIENTE, nvl(COUNT(p.CODIGO_CLIENTE),0)
FROM CLIENTE c, PEDIDO p 
WHERE c.CODIGO_CLIENTE = p.CODIGO_CLIENTE (+)
...
--4 Devuelve el nombre del cliente, el nombre y primer apellido de su representante de ventas y el número de teléfono
--de la oficina del representante de ventas, de aquellos clientes que no hayan realizado ningún pago.
SELECT c.CODIGO_CLIENTE, c.NOMBRE_CLIENTE, e.NOMBRE, e.APELLIDO1, o.TELEFONO
